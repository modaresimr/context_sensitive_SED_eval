...
def compute_mme(ground_truth, metadata, predictions, **kwargs):
    from . import mme as m
    ev = m.eval(ground_truth, predictions, metadata, **kwargs)
...
def get_single_result(gtf, pef, metaf=None, psdsf=None, **kwargs):
    ...
    # print(meta_df)
    return get_single_result_df(groundtruth, predictions, meta_df, **kwargs)


def get_single_result_df(groundtruth, predictions, meta_df=None, psdsf=None,**kwargs):
    out = {}
    if meta_df is None:
    ...
    mme = compute_mme(groundtruth, metadata, predictions, **kwargs)
    for m in mme:
